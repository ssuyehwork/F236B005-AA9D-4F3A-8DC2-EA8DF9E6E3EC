...
if __name__ == '__main__':
    ...
    main_window = QWidget()
    main_window.setWindowTitle("SearchLineEdit 测试")
    main_window.setGeometry(300, 300, 500, 250)

    layout = QVBoxLayout(main_window)
    layout.setContentsMargins(20, 20, 20, 20)

    # 标题
    title = QLabel("搜索框历史记录测试")
    ...
    for entry in test_entries:
        search_box.add_history_entry(entry)
    ...
    def on_search():
        ...
        if term:
            ...
            result_label.setText(f"搜索结果: {term}")

    search_box.returnPressed.connect(on_search)

    # 结果显示
    result_label = QLabel("在搜索框中输入内容并按回车进行搜索")
    ...
    """)
    clear_btn.clicked.connect(lambda: (search_box.clear_history(),
                                       result_label.setText("历史记录已清空")))
