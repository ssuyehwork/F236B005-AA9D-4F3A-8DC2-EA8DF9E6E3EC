...
- `main_window`: `MainWindow` 的实例。
- `quick_window`: `QuickWindow` 的实例。
...
- **`start()`**:
  ...
  - **执行流程**:
    ...
    2.  创建主窗口 `MainWindow` (此时不显示)。
    3.  创建并显示悬浮球 `FloatingBall`。悬浮球的各种请求信号 (如显示快速窗口、显示主窗口、退出应用) 会被连接到 `AppManager` 中对应的槽函数。
    4.  从设置中加载并恢复悬浮球上次的位置。
    5.  创建快速窗口 `QuickWindow` (此时不显示)。
- **`show_quick_window()`**:
  - **功能**: 显示并激活快速窗口。确保窗口不在最小化状态，并将其带到前台。
- **`show_main_window()`**:
  - **功能**: 显示并激活主窗口。
- **`on_main_window_closing()`**:
  - **功能**: 当主窗口被用户关闭时，此方法被调用。它的作用是**隐藏**主窗口而不是退出整个应用，应用的生命周期由悬浮球的“退出”选项控制。
- **`quit_application()`**:
...
- **`closeEvent(event)`**:
  ...
  - **逻辑**:
    ...
    2.  调用 `self.hide()` **隐藏窗口**，而不是真正关闭它。
    3.  调用 `event.ignore()` 阻止事件继续传播，防止应用退出。
